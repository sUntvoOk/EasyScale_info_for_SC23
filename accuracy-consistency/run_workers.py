...
validating_configs = {
  #stage_0': Profiling.generate_a_proposal(placement={'v100': 4, 'p100': 0, 't4': 0}, threads={'v100': 1, 'p100': 0, 't4': 0}, mp=4), 
  #'stage_1': Profiling.generate_a_proposal(placement={'v100': 2, 'p100': 0, 't4': 0}, threads={'v100': 2, 'p100': 0, 't4': 0}, mp=4), 
  #'stage_2': Profiling.generate_a_proposal(placement={'v100': 1, 'p100': 0, 't4': 0}, threads={'v100': 4, 'p100': 0, 't4': 0}, mp=4), 
  #'stage_2': Profiling.generate_a_proposal(placement={'v100': 1, 'p100': 2, 't4': 0}, threads={'v100': 2, 'p100': 1, 't4': 0}, mp=4), 

  'workers': Profiling.generate_a_proposal(placement={'v100': 1, 'p100': 0, 't4': 0}, threads={'v100': 8, 'p100': 0, 't4': 0}, mp=8), 
}
  ...
  def run():
    ...
    #for stage in ['stage_0']:
      ...
      if determinism_level == 0:
        ...
        # hack
        #path = os.path.join('./logs_homo', application_name)
        path = os.path.join('./logs_workers', application_name)
      if determinism_level == 12:
        path = os.path.join('./logs_hete', application_name)
      # hack
