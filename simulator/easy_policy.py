...
class Intrajob(object):
    def __init__(self, max_parallelism, gpu_types, gpu_flops, gpu_perf, gpu_mems, gpu_nums, interference=None):
        ...
        # TODO: 目前并未考虑多process情况下的干扰
        # 考虑多种processes设置下的interference，例如 最大为3则需要考虑1,2,3三种情况
        # 考虑interference导致的浪费
        ...
        #self._processes = tuple([1 for i in range(len(self.types))])
        self.processes_candidates = [(1,1,1)]
        ...
        if interference:
            ...
            # TODO: fix this hack for v100(32G), p100(16G), and t4(16G)
            self.processes_candidates = [(2,1,1), (1,1,1)]
            # determine gpu_perf under interference
            self.perf = tuple([a*b for a,b in zip(self.perf, self.interference)])
            # TODO: assume no interference right now, i.e., interference==(1,1,1)
    ...
    def get_perf_candidates(self):
        ...
        logger.debug('## all perf combinations are {}'.format(perf_combinations))
        ...
        # sort perf_candidates according to the first gpu type
        perf_candidates = sorted(perf_candidates, key=lambda x:x[0][0])
        self.perf_candidates = tuple(perf_candidates)
        logger.debug('## all perf combinations (final) are {}'.format(perf_candidates))

    def get_proposals(self):
        proposals = []
        for perf, m in self.perf_candidates:
            logger.debug('## current perf candidate {}, nums {}'.format(perf, self.nums))

            # TODO: generate processes_candidates automatically
            processes_candidates = self.processes_candidates
            for processes in processes_candidates:
                combinations = self.combinationSum(perf=perf, processes=processes)
                logger.debug('## combinations {}'.format(combinations))
                perf_scaled = tuple([p*m for p in self.perf])
                for comb in combinations:
                    logger.debug("## perf_scaled {}, perf {}, comb {}".format(perf_scaled, perf, comb))
                    w, threads = self.waste(theory=perf_scaled, actual=perf, comb=comb, mp=self.mp, processes=processes)
                    logger.debug("## w {}, threads {}".format(w, threads))
                    proposals.append([comb, w, threads, self.mp, processes])

            logger.debug("## len(proposals) is {}".format(len(proposals)))
    ...
    def combinationSum(self, perf, processes):
        nums = self.nums
        target = self.mp
        ...
        logger.debug("## candidates are {}".format(candidates))
        ...
        def dfs(begin, target):
            ...
            for i in range(begin, len(candidates)):
                ...
                if i > begin and candidates[i] == candidates[i-1]:
                    continue
                temp.append(candidates[i])
                dfs(i+1, target-candidates[i][0]*processes[gpu_idx])
                temp.pop()
        dfs(0, target)
    ...
    def _convert_to_final_proposals(self, proposals):
        merged = []
        for p in proposals:
            # p[0]: comb; p[1]: waste; p[2]: threads; p[3]: mp; p[4]: processes
            ...
            for i in range(len(self.types)):
                if cnt[i] == 0:
                    processes[i] = 0
            merged.append([cnt, p[1], p[2], flops, speedup, p[3], processes])

        merged = sorted(merged, key=lambda x:x[0][0], reverse=True)
        merged = sorted(merged, key=lambda x:x[4], reverse=True)

        logger.debug("## len(merged) {}".format(len(merged)))

        # TODO: ?
        # 会面临多个process和thread的倾向性问题
        # 在仅有one gpu type时，我们倾向于选择多个线程，以避免任何进程间干扰
        final = OrderedDict()
        for p in merged:
            key = tuple(p[0])
            key = (tuple(p[0]), tuple(p[-1])) # placement, processes
            #value = [p[1], p[2], p[3], p[4], p[5]]
            value = [vv for i,vv in enumerate(p) if i > 0]
            if final.__contains__(key):
                #if p[4] > final[key][0] and all([ii<=0 for ii in value[5]]):
                if p[4] > final[key][0]:
                    final[key] = value
            else:
                final[key] = value
        ...
        for k, v in final.items():
            placement = dict(zip(self.types, k[0]))
            processes = dict(zip(self.types, v[5]))
    ...
    def waste(theory, actual, comb, mp, processes):
        logger.debug('@@ waste input : theory {}, actual {}, comb {}, mp {}, processes {}'.format(theory, actual, comb, mp, processes))
        ...
        logger.debug("## scales {}".format(scales_temp))
        
        # if there is only one gpu type
        is_single_type = False
        cnt_gpu_nums = cnt.copy()
        logger.debug("## cnt_gpu_nums {}".format(cnt_gpu_nums))
        if len(list(filter(lambda x: x>0, cnt_gpu_nums))) == 1:
            ...
            for idx,_ in enumerate(cnt):
                if cnt[idx] > 0:
                    gpu_idx = idx
                    break
            logger.debug("## gpu_idx {}".format(gpu_idx))
        ...
        logger.debug("## ref is {}".format(ref))
...
if __name__ == '__main__':
    ...
    intra.get_perf_candidates()
    #for c in intra.perf_candidates:
    #    print(c)

    #intra.perf_candidates = [((40,31,13),1)]
    #intra.perf_candidates = [((7,4,2),2)]
